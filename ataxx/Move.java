...
class Move {
    ...
    boolean isJump() {
        return !isExtend() && distance() <= 2;
    }

    /** Return the distance between my current square to my target square. */
    int distance() {
        int rowDiff = abs(_row0 - _row1);
        int colDiff = abs(_col0 - _col1);

        return (int) sqrt(pow(rowDiff, 2) + pow(colDiff, 2));
    }
